...
function normalizeTree(folder: Folder): void {
  // unwrap single wrapper folders
  let changed = true
  while (changed) {
    changed = false
    if (folder.children.length === 1 && folder.bookmarks.length === 0) {
      const only = folder.children[0]
      if (WRAPPER_NAMES.has(only.name.trim().toLowerCase())) {
        folder.bookmarks = only.bookmarks
    ...
    for (const c of folder.children) {
      ...
      if (WRAPPER_NAMES.has(name) && c.bookmarks.length === 0 && c.children.length > 0) {
        // promote grandchildren
        for (const gc of c.children) kept.push(gc)
        changed = true
      } else if (WRAPPER_NAMES.has(name) && c.bookmarks.length > 0 && c.children.length === 0) {
        // merge bookmarks up
        folder.bookmarks.push(...c.bookmarks)
        c.bookmarks = []
        changed = true
      } else {
        kept.push(c)
...
function importCsv(text: string) {
  ...
  const iCreated = idx('created') >= 0 ? idx('created') : idx('add date')

  let added = 0
  for (let i = 1; i < lines.length; i++) {
    const cols = parseCsvLine(lines[i])
    if (cols.length <= iUrl) continue
    const url = (cols[iUrl] || '').trim()
    if (!url || !url.startsWith('http')) continue
    const title = iTitle >= 0 ? (cols[iTitle] || url) : url
    const folderPath = iFolder >= 0 ? (cols[iFolder] || '') : ''
    const tags = iTags >= 0 ? (cols[iTags] || '').split(/[,，]/).map((t) => t.trim()).filter(Boolean) : []
...
async function loadFromSite() {
  try {
    const res = await fetch('/bookmarks-data.json?' + Date.now())
    if (!res.ok) throw new Error('站点还没有 bookmarks-data.json，请先在本机整理并发布一次')
    const data = await res.json()
    if (!data || !data.id) throw new Error('数据格式无效')
    if (!confirm('将用站点上的书签覆盖本机管理器数据，是否继续？')) return
    root.value = data
    selectedFolderId.value = 'root'
    flash('已从站点加载书签')
  } catch (e: any) {
    flash('加载失败：' + (e?.message || e))
  }
}
...
function buildMdFiles(folder: Folder, prefix: string, files: { path: string; content: string }[]) {
  ...
  for (const c of folder.children) {
    ...
    let i = 1
    while (used.has(slug)) {
      slug = slugify(c.name) + '-' + i++
    }
    used.add(slug)
    const path = prefix ? `${prefix}/${slug}` : slug
    childLinks.push({ name: c.name, slug, count: countAll(c) })
    buildMdFiles(c, path, files)
  }

  const content = folderToMd(folder, childLinks)
...
function exportMarkdown() {
  ...
  function walk(f: Folder, path: string) {
    if (f.bookmarks.length) {
      parts.push(`## ${path || f.name}`)
      for (const b of f.bookmarks) {
        const tags = b.tags.length ? ' ' + b.tags.map((t) => '`' + t + '`').join(' ') : ''
        parts.push(`- [${b.title}](${b.url})${tags}`)
...
async function publishToGitHub() {
  ...
  try {
    ...
    payload.push({
      path: 'docs/public/bookmarks-data.json',
      content: toBase64FromString(JSON.stringify(root.value)),
      message: 'chore(bookmarks): data snapshot for manager',
    })
  ...
  <div class="bm">
    <header class="bm-header">
      ...
      <div class="bm-actions">
        ...
        <button class="bm-btn" type="button" @click="loadFromSite">从站点加载</button>
...
<style scoped>
.bm { max-width: 1100px; margin: 0 auto; }
.bm-header {
...
.bm-title { margin: 0; font-size: 1.35rem; font-weight: 700; color: var(--ol-text, #111); }
.bm-sub { margin: 0.25rem 0 0; font-size: 0.85rem; color: var(--ol-text-secondary, #6b7280); }
.bm-status { margin-left: 0.5rem; color: var(--ol-primary, #2563eb); }
.bm-actions { display: flex; flex-wrap: wrap; gap: 0.5rem; }
.bm-btn {
  border: 1px solid var(--ol-border, #e5e7eb); background: var(--ol-card, #fff); color: var(--ol-text, #111);
  border-radius: 8px; padding: 0.35rem 0.75rem; font-size: 0.82rem; cursor: pointer;
}
.bm-btn:hover { border-color: var(--ol-primary, #2563eb); color: var(--ol-primary, #2563eb); }
.bm-btn-primary { background: var(--ol-primary, #2563eb); color: #fff; border-color: transparent; }
.bm-btn-primary:hover { opacity: 0.9; color: #fff; }
.bm-btn-danger { color: #dc2626; }
.bm-btn-danger:hover { border-color: #dc2626; }
.bm-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.bm-move-bar {
  display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center;
  padding: 0.65rem 1.5rem; background: var(--vp-c-bg-soft, #f4f4f5); border-bottom: 1px solid var(--ol-border, #e5e7eb);
}
...
.bm-sidebar {
  border-right: 1px solid var(--ol-border, #e5e7eb); padding: 0.75rem 0;
  background: var(--vp-c-bg-soft, #f4f4f5);
}
.bm-side-head {
  display: flex; justify-content: space-between; align-items: center;
  padding: 0 0.85rem 0.5rem; font-size: 0.8rem; font-weight: 600; color: var(--ol-text-secondary, #6b7280);
}
.bm-icon-btn {
  border: none; background: transparent; cursor: pointer; font-size: 1.1rem; color: var(--ol-primary, #2563eb);
}
.bm-inline-form { display: flex; gap: 0.35rem; padding: 0 0.6rem 0.6rem; }
.bm-inline-form input {
  flex: 1; border: 1px solid var(--ol-border, #e5e7eb); border-radius: 6px; padding: 0.3rem 0.5rem; font-size: 0.8rem;
  background: var(--ol-card, #fff); color: var(--ol-text, #111);
}
.bm-tree { list-style: none; margin: 0; padding: 0; }
.bm-tree-item {
  display: flex; align-items: center; gap: 0.35rem; padding: 0.35rem 0.6rem; cursor: pointer;
  font-size: 0.85rem; color: var(--ol-text, #111);
}
.bm-tree-item:hover { background: var(--ol-bg, #fafafa); }
.bm-tree-item.active { background: var(--vp-c-brand-soft, rgba(37, 99, 235, 0.1)); color: var(--ol-primary, #2563eb); font-weight: 600; }
...
.bm-tree-ops { display: none; gap: 0.15rem; }
.bm-tree-item:hover .bm-tree-ops { display: flex; }
.bm-tree-ops button {
  border: none; background: transparent; cursor: pointer; font-size: 0.7rem; color: var(--ol-text-secondary, #6b7280); padding: 0 0.2rem;
}
.bm-tree-ops button:hover { color: var(--ol-primary, #2563eb); }
.bm-main { padding: 1rem 1.25rem; }
.bm-main-bar {
  display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center; margin-bottom: 0.85rem;
}
...
.bm-search {
  ...
  font-size: 0.85rem; background: var(--ol-card, #fff); color: var(--ol-text, #111); min-width: 140px;
}
.bm-subfolders {
